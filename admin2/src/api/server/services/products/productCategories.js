...
class ProductCategoriesService {
	...
	async getCategories(params = {}) {
		...
		const items = await db
	      ...
	      const count = await this.countProductsByCategory(item._id)
	      item.count = count
	    }));
    ...
    return db.collection('products').countDocuments({$or:[{category_id: categoryId},{category_ids:{$in:[categoryId]}}]}).then(count => {
      return count
    });
  }
